...
exports.listReceita = (req, res, next) => {
    ...
    mysql.getConnection((error, conn) => {
        ...
        if (error) { return res.status(500).send({ error: error }) }
        conn.query(
            `SELECT id_Receita, dt_Emissao, nome, Quantidade, dosagem, rc.dt_Validade FROM tbl_Receita AS rc
...
exports.detalhesReceita = (req, res, next) => {
    ...
    mysql.getConnection((error, conn) => {
        ...
        conn.query(`
        SELECT rm.id_Remedio, rm.bula, rm.descricao, rm.preco, rm.tarja, rc.dt_Emissao, rc.dt_Validade, rc.dosagem, rc.orientacoes, pc.nome AS Paciente, pc.cpf, pc.rg, pc.dt_Nascimento, pc.alergia, pc.foto, rr.Quantidade , md.nome AS Medico, md.dt_Nascimento as dt_Medico, md.especialidade, md.crm, rm.nome AS Remedio
